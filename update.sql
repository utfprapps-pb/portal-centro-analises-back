...
create table tb_partner (status smallint check (status between 0 and 1), id bigserial not null, cnpj varchar(255) not null unique, name varchar(255), primary key (id));
...
create table tb_student_professor (approved boolean, created_at date, id bigserial not null, professor bigint unique, student bigint unique, primary key (id));
...
create table tb_user (balance numeric(38,2), email_verified boolean, role smallint check (role between 0 and 4), status smallint check (status between 0 and 1), created_at timestamp(6), id bigserial not null, partner_id bigint, updated_at timestamp(6), cpf_cnpj varchar(255), email varchar(255), name varchar(255), password varchar(255), ra_siape varchar(255), primary key (id), constraint set_unique_email unique (email));
...
alter table if exists tb_user add constraint FKbqplyqefdhku3s1nop5h9r4i7 foreign key (partner_id) references tb_partner;create sequence tb_email_log_seq start with 1 increment by 50;
create table project_student (project_id bigint not null, user_id bigint not null);
create table tb_analysis (equipment_id bigint, id bigserial not null, description varchar(255), name varchar(255), primary key (id));
create table tb_domain_role (role smallint check (role between 0 and 4), id bigserial not null, domain varchar(255), primary key (id), constraint unique_domain_role unique (domain));
create table tb_email_code (created_at timestamp(6), id bigserial not null, user_id bigint unique, validate_at timestamp(6), code varchar(255), primary key (id));
